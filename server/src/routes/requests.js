...
knl.post('requests', async (req, resp) => {
  const schema = Joi.object({
    fkClients: Joi.number().min(1).required(),
    DateEmission: Joi.date().raw().required(), 
    DateDelivery: Joi.date().raw().required(),
    fkAddress: Joi.number().min(1).required(),
    total: Joi.number().min(1).max(9999999999).required(),
    prodRequests: Joi.array().items(
        Joi.object({
